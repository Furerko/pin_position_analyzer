...
# ============================================================
# PIN POSITION SIMPLE VIEW V4
# W raporcie jest tylko prosty widok:
# Pozycja pinów | Odchylenie pinów | Oś X | Oś Y | Oś Z
# ============================================================
...
# Ile pinów z największym problemem ma być oznaczone jako „najbardziej odchylone”.
# Jeśli chcesz pokazać więcej, zmień np. na 5 albo 8.
TOP_BAD_PINS_PER_FILE = 4

# AUTO_LAYOUT_MODE działa dla innych produktów z większą liczbą pinów.
# Program bierze listę pinów i dzieli ją na dwie kolumny:
# np. 8 pinów  -> [1,5], [2,6], [3,7], [4,8]
# np. 16 pinów -> [1,9], [2,10], ... [8,16]
# np. 24 piny -> [1,13], [2,14], ... [12,24]
# Jeśli liczba pinów jest nieparzysta, ostatni pin zostanie pokazany sam, np. [9].
...
def make_auto_two_column_layout(active_pins):
    """
    Automatyczny layout dla produktów z dowolną liczbą pinów.

    Logika:
    - sortuje piny rosnąco,
    - dzieli na dwie kolumny,
    - paruje pierwszy pin z pierwszej połowy z pierwszym pinem z drugiej połowy.

    Przykłady:
    8 pinów  -> [1,5], [2,6], [3,7], [4,8]
    16 pinów -> [1,9], [2,10], ... [8,16]
    24 piny -> [1,13], [2,14], ... [12,24]
    9 pinów  -> [1,6], [2,7], [3,8], [4,9], [5]
    """
...
def get_layout_for_ref(ref, active_pins):
    ...
    # 1) Najpierw sprawdzamy ręczną mapę produktu, np. GSR/SWEET/HKMC.
    ...
    # 2) Jeśli produkt nie ma ręcznej mapy, używamy automatu dla dowolnej liczby pinów.
    if AUTO_LAYOUT_MODE == "TWO_COLUMNS":
        return make_auto_two_column_layout(active)

    # 3) Ostateczny fallback: każdy pin osobno.
...
def analyze(records):
    ...
    for group, pins in active_pins.items():
        ...
        # pin -> osie gdzie pin miał odchylenie
        bad_by_axis = {
            "X": set(),
            "Y": set(),
            "Z": set(),
        ...
        # pin -> największa odchyłka liczbowo
        ...
        # wybieramy najgorsze piny z pliku
        top_bad_pins = set(
        ...
        # ogólne odchylenie pinów = top najgorsze piny, żeby widok był czysty
    ...
    for col in ws.columns:
        ...
        for cell in col:
            if cell.value is not None:
                width = max(width, min(45, len(str(cell.value)) + 4))
...
def style_simple_sheet(ws):
    ...
    # podświetlenie komórek z odchyłkami
    for row in range(1, ws.max_row + 1):
...
def print_console_view(analysis_result):
    ...
    for (file, ref), data in sorted(analysis_result.items()):
        print(f"\nPLIK: {file} | REF: {ref}")
        print("-" * 90)
        print(f"{'Pozycja pinów':<18} | {'Odchylenie pinów':<20} | {'Oś X':<12} | {'Oś Y':<12} | {'Oś Z':<12}")
        print("-" * 90)
        ...
        for row_name, pins in layout.items():
            ...
            print(f"{pos:<18} | {overall:<20} | {axis_x:<12} | {axis_y:<12} | {axis_z:<12}")
...
def main():
    ...
    output_file = os.path.join(out_dir, f"pin_simple_view_V4__{timestamp}.xlsx")
